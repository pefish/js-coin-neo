...
export default class NeoWalletHelper extends BaseCoin {
  ...
  getAssertIdByName (name) {
    return ASSETS[name] || null
  }

  getNameByAssertId (id) {
    for (let [name, assetId] of Object.entries(ASSETS)) {
      if (assetId === id) {
        return name
      }
    }
  ...
  async sendAssets (privateKey, targetAddress, neoAmount, gasAmount, network = `testnet`) {
    ...
    const intent = api.makeIntent({ NEO: neoAmount, GAS: gasAmount }, targetAddress)
